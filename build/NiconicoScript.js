...
class SearchVideoPager extends VideoPager {
    constructor(query) {
        const res = local_http.POST(URL_SEARCH, `q=${encodeURIComponent(query)}`, {
            "Content-Type": "application/x-www-form-urlencoded",
        }, false);
        if (!res.isOk) {
            throw new ScriptException(`Failed request [${URL_SEARCH}] (${res.code})`);
        }
        const nicoVideos = JSON.parse(res.body);
...
//#endregion
//#region content
function getContentDetails(videoUrl) {
    ...
    const [videoXMLRes, videoHTMLRes] = local_http.batch().GET(getThumbInfoUrl, {}, false).GET(videoUrl, {}, false).execute();
...
function getComments(videoUrl) {
    ...
    const videoCommentsRes = local_http.POST(URL_COMMENTS, JSON.stringify({
    ...
    const comments_response = JSON.parse(videoCommentsRes.body);
...
class NiconicoCommentPager extends CommentPager {
    constructor() {
        super([], false);
    }
}
...
class ChannelContentsPager extends VideoPager {
    constructor(channel_url) {
        ...
        const res = local_http.GET(searchUrl, {
            "X-Frontend-Id": "6",
        }, false);
        if (!res.isOk) {
            throw new ScriptException(`Failed request [${searchUrl}] (${res.code})`);
        }
        const channel_videos_response = JSON.parse(res.body);
...
function getPlaylist(playlist_url) {
    ...
    const [playlistHttpRes, playlistApiRes] = local_http
        .batch()
        .GET(playlist_url, { "X-Frontend-Id": "6" }, true)
        .GET(playlistApiUrl, { "X-Frontend-Id": "6" }, true)
        .execute();
    ...
    const playlist_response = JSON.parse(playlistApiRes.body);
...
function getUserPlaylists() {
    ...
    const playlistUrls = user_playlists_response.data.mylists.map((playlist) => `https://www.nicovideo.jp/my/mylist/${playlist.id}`);
    return playlistUrls;
}
function getUserSubscriptions() {
    ...
    const res = local_http.GET(URL_FOLLOWING, {
        "X-Frontend-Id": "6",
    }, true);
    if (!res.isOk) {
...
function nicoVideoDetailsToPlatformVideoDetails(videoXML, hlsEndpoint) {
    ...
    return new PlatformVideoDetails({
        ...
        shareUrl: videoUrl,
        author: new PlatformAuthorLink(new PlatformID(PLATFORM, authorId, plugin.config.id), queryVideoXML("user_nickname"), `https://www.nicovideo.jp/user/${authorId}`, queryVideoXML("user_icon_url")),
...
function nicoSearchVideoToPlatformVideo(v) {
    ...
    return new PlatformVideo({
        ...
        shareUrl: videoUrl,
        author: new PlatformAuthorLink(new PlatformID(PLATFORM, authorId, plugin.config.id), "ニコニコ", `https://www.nicovideo.jp/user/${authorId}`, "https://play-lh.googleusercontent.com/_C1KxgGIw43g2Y2G8salrswvYqkkBum5896cCrFOWkgdAxZI10efI-oQxfWRfLOBysE"),
    });
}
function nicoVideoToPlatformVideo(v) {
    ...
    return new PlatformVideo({
        ...
        shareUrl: videoUrl,
        author: new PlatformAuthorLink(new PlatformID(PLATFORM, v.owner.id, plugin.config.id), v.owner.name, `https://www.nicovideo.jp/user/${v.owner.id}`, v.owner.iconUrl),
    });
}
...
function fetchHLSEndpoint(videoId, actionTrackId, accessRightKey) {
    ...
    const res = local_http.POST(url, JSON.stringify({
    ...
    const hls_response = JSON.parse(res.body);
...
//# sourceMappingURL=http://localhost:8080/build/NiconicoScript.js.map
